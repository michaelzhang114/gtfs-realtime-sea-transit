    ...
    if content_type == 'application/json':
        ...
        with conn:
            with conn.cursor() as cursor:
                cursor.execute("INSERT INTO users (name, email) VALUES (%s, %s) RETURNING id;", (my_name, my_email))
                user_id = cursor.fetchone()[0]
        return {"message": f"User {my_name} created with {my_email}.", "id": user_id, "name": my_name, "email": my_email}, 201
        ...
        with conn.cursor() as cursor:
            ...
            if users:
                ...
                for user in users:
                    ...
                    result.append({"id": user[0], "name": user[1], "email": user[2], "routes": routes_data})
...
def get_user(user_id):
    ...
    with conn:
        with conn.cursor() as cursor:
            ...
            if user:
                return jsonify({"id": user[0], "name": user[1], "email": user[2], "routes": routes_data})
            else:
    ...
    with conn:
        with conn.cursor() as cursor:
            ...
            if cursor.rowcount == 0:
                return jsonify({"error": f"User with ID {user_id} not found."}), 404
    return jsonify({"id": user_id, "name": my_name, "message": f"User with ID {user_id} updated.", "email": my_email})
